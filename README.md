...
**一句话结论**：默认 prompt 下 AI 覆盖面更强（+24.3pp）却**一个 bug 都没抓到**，还把 bug 写成期望值。
补上三条指令后（给规格 / 维度交叉 / **枚举特征进入路径**），**命中 17 个**——见下方 run02–run04。
...
详见 [results/run04_结果.md](results/run04_结果.md)。

| 指标 | run01 源码 | run02 规格 | run03 +交叉 | **run04 +路径** |
|---|---|---|---|---|
| **命中真 bug** | ❌ 0 | ❌ 0 | ❌ 0 | ✅ **17** |
| 误报 | 7 | 2 | 17 | **6** |
| bug 固化 | 🔴 2 | ✅ 0 | ✅ 0 | ✅ 0 |
...
```
① 只给规格，不给源码 —— 期望值来自「应该返回什么」
② 枚举特征维度，构造两两交叉
③ 对每个特征枚举「进入路径」：由参数产生 vs 由输入自带
   同一特征的两条路径，函数内部往往是「生成」和「解析」两套代码
```

issue #290 的根因正则**只在解析输入时被调用**，只有路径 B 能触发。前三轮全部只覆盖路径 A。

### 反直觉副产物：命中率与杀伤率脱钩

杀伤率最高的 run01/run02（81.1%）一个 bug 没抓到；抓到 17 个的 run04 只有 67.6%。

> **覆盖率和变异杀伤率测的不是「能不能发现 bug」。用它们当质量门禁的团队值得看一眼这张表。**

## run05：跨项目可迁移性 —— ✅ 验证通过

指令一字未改，换一个完全不同类型的项目。详见 [results/run05_结果.md](results/run05_结果.md)。

| | run01–04 | run05 |
|---|---|---|
| 项目 | pypinyin（汉字转拼音） | **cachetools（缓存容器）** |
...
命中的是 `test_tlru_overwrite_with_already_expired_ttu_pathB` —— **又是指令③（进入路径）产出的用例**。
...
- [x] ~~run04：补「特征进入方向」指令~~ ✅ 已命中
- [x] ~~换项目验证可迁移性~~ ✅ run05 通过
